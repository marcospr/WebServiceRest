...
public class ServiceController {
	...
	@Path("/cadastrar")
	public Response Cadastrar(@Context UriInfo uriInfo, Pessoa pessoa) {
		...
		try {
			...
			URI location = URI.create(uriInfo.getBaseUri().getPath() + "service/getPessoa/" + entity.getCodigo());
			return Response.created(location).build(); // "Registro cadastrado
														// com sucesso!";

		} catch (Exception e) {

			return Response.serverError().build(); // "Erro ao cadastrar um
													// registro " +
													// e.getMessage();
		}
